...
export default function PublicLessonsPage() {
  ...
  useEffect(() => {
    api.get<Story[]>("/stories?mode=questions").then(setStories);
  }, []);
    ...
    <Flex as="main" direction="column" align="center" px={6} py={16} bg="bg.page" minH="100vh">
      <Box w="full" maxW="3xl">
        <Heading as="h1" fontSize="3xl" fontWeight="bold" color="fg.heading" mb={2}>
          {t("publicStories.lessonsTitle")}
        </Heading>
        <Text color="fg.subtle" mb={4}>
          {t("publicStories.lessonsSubtitle")}
        </Text>
        <StoryTypeTabs active="questions" freeformHref="/stories" questionsHref="/lessons" />

        {!stories ? (
          <Text color="fg.subtle">{t("common.loading")}</Text>
        ) : stories.length === 0 ? (
          <Text color="fg.subtle">{t("publicStories.lessonsEmpty")}</Text>
        ) : (
